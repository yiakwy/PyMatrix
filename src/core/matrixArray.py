...
class matrixArrayLists(list):
    ...
    def nil_multi(self, *args):
        ...
        # set root node
        root  = [None for i in range(0, args[0])]
        # initialize a queue
        queue = []
        queue.append((root,0))
    ...
    def index_value(self, key):
        ...
        if  isinstance(key, tuple):
            return ([self.index_value(i)[0] for i in key],)         
...
#               print(self)
    ...
    def setitem_multi(self, 
        ...
        if   isinstance( key, int  ):
            ...
            def routines(curr, key, index):
                ...
                if  curr == depth:
                    for i in   key[curr - 1]:
                        ...
                        # use settiem   
                        try:
                            # this is wrong, if value is not vector
                            if   switch:      
                                self.setitem(tuple(index), itv.__next__())
                                continue
                            if   not row_transforamtion_flag and not col_transforamtion_flag:
                                try:
                                    self.setitem(tuple(index), self.__class__(value)[tuple(index[0:2])].matrix2list())
                                except AttributeError:
                                    self.setitem(tuple(index), self.__class__(value)[tuple(index[0:2])])
                            elif row_transforamtion_flag and not col_transforamtion_flag:
                                self.setitem(tuple(index), self.__class__(value)[tuple(0, index[1])])
                            elif not row_transforamtion_flag and col_transforamtion_flag: 
                                self.setitem(tuple(index), self.__class__(value)[tuple(index[0], 0)])   
            ...
            if    len(key) < 2:
                ...
                row_transforamtion_flag = False
                col_transforamtion_flag = False
            elif  len(key[0]) == 1 and isinstance(key[1], list) and len(key[1]) != 1:
                row_transforamtion_flag = True
                col_transforamtion_flag = False
            elif  isinstance(key[1], int):
                row_transforamtion_flag = False
                col_transforamtion_flag = True
            elif  len(key[0]) != 1 and isinstance(key[1], list) and len(key[1]) == 1:
                row_transforamtion_flag = False
                col_transforamtion_flag = True
            else:
    ...
    def __setitem__(self, key, value): 
        ...
        if  isinstance(value, self.__class__):
            values  =  value.get_runtime_list()
       
        if  isinstance(values, list): 
            self.setitem_multi(key, values)
        else:
        ...
        if  isinstance(key, int):  
            try:                   
                ...
                if    flag and len(size) == 2 and size[0] == 1:
                    ...
                    result = self(key)#super(matrixArrayLists, self).__getitem__(key)#how can we get sub matrixArray, i.e. mat is result : True
    ...
    def _element2str(self, i, j, l=[], out=None):
        formatter =  self.formatter; param = {'float': formatter.float, 'width': formatter.width[j]}
